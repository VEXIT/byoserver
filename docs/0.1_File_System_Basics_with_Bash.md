...
## 7. Create a script file

```bash
touch test.sh
```

**Explanation:** `touch` creates an empty file; here we use it to create a new script file `test.sh`.
...
## 10. Add a name prompt to the script
...
## 11. Go up two levels to your home directory
...
## 13. Copy the project folder

```bash
cp -r development/my-project development/my-project2
```
...
## 14. Remove the original project folder

```bash
rm -r development/my-project
```
...
You now have `development/my-project2` with your `HelloWorld.sh` script. You have used: creating and changing directories (`mkdir`, `cd`), creating and deleting files (`touch`, `rm`), renaming (`mv`), editing (`nano`), running a script (`chmod +x`, `./script`), listing contents (`ls`, `ls -l`, `ls -la`), copying a folder (`cp -r`), and going up with `..`.
